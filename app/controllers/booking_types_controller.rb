class BookingTypesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @booking_type.save
        @booking_types = BookingType.all.order(created_at: :desc)
        format.turbo_stream
  ...
  def provider_services
    ...
    @providerservices = provider.booking_types.pluck(:name, :id)
    respond_to do |format|
      format.turbo_stream
    end
  end
  ...
  private
    ...
    def booking_type_params
      params.require(:booking_type).permit(:user_id, :cost_id, :duration, :name, :free)
    end
end
